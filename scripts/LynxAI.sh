...
# this script applies the LynxAIeu patches to llama-swap
...
patch="${patch:-0}"
branch="${branch:-lynx-$digits.$patch}"
...
(
    set -x
    git status
    git commit -m 'README: explain the LynxAIeu fork' README.md
    git status
)
...
(
    log "replace mostlygeek/llama-swap -> LynxAIeu/llama-swap"
    set -x
    git status
    sed -i -e 's,module github.com/mostlygeek/llama-swap,module github.com/LynxAIeu/llama-swap,' go.mod
    git add go.mod
    find -name "*.go" -exec sed -i -e 's,"github.com/mostlygeek/llama-swap,"github.com/LynxAIeu/llama-swap,' {} + -exec git add {} +
    git status
    go run . -h 2>/dev/null # smoke test
    git commit -m "fork mostlygeek -> LynxAIeu"
)
...
(
    ...
    git status
    rm go.sum go.mod
    go mod init github.com/LynxAIeu/llama-swap
    go mod tidy
    go run . -h 2>/dev/null # smoke test
    git status
...
(
    log "remove .github directory to disable GitHub workflow"
    set -x
    git status
    rm .github -rf
    git status
    git commit -m "remove .github directory to disable GitHub workflow" .github
)

(
    log "add LynxAI.sh and LynxAI.patch"
    set -x
    git status
    # we keep the original LynxAI.sh LynxAI.patch as untracked in another folder
    # because Git will remove them when switching to another branch
    pwd
    cp -fv "$dir"/LynxAI.sh "$dir"/LynxAI.patch scripts/
    chmod +x scripts/LynxAI.sh
    git add  scripts/LynxAI.sh scripts/LynxAI.patch
    git commit -m 'scripts: add LynxAI.sh and LynxAI.patch'
)
...
(
    log "merge branch $branch into lynx (lynx is the default branch name of the LynxAIeu fork)"
    set -x
    git fetch origin
    #git reset --hard  origin/lynx
    git switch -C lynxai origin/lynx    
    #git branch -u origin/lynx lynx    # set upstream tracking (already done by `switch -C`)
    git status
    git merge -X theirs --rerere-autoupdate --verbose --stat --progress --autostash -m "Merge branch '$branch' into lynx" "$branch"
)
