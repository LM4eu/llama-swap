...
# this script applies the lynx patches to llama-swap
...
(
    set -x
    git status
    git commit -m 'README: explain the lynx fork' README.md
    git status
)
...
(
    log "add lynx.sh and lynx.patch"
    set -x
    git status
    # we keep the original lynx.sh lynx.patch as untracked in another folder
    # because Git will remove them when switching to another branch
    pwd
    cp -fv "$dir"/lynx.sh "$dir"/lynx.patch scripts/
    chmod +x scripts/lynx.sh
    git add  scripts/lynx.sh scripts/lynx.patch
    git commit -m 'scripts: add lynx.sh and lynx.patch'
)
...
(
    log "merge branch $branch into lynx (default branch name of the LynxAIeu fork)"
    set -x
    git fetch origin
    #git reset --hard  origin/lynx
    git switch -C lynx origin/lynx    
